...
namespace 学生成绩管理系统
{
    public partial class Admin : Form
    {
        ...
        private void 课程添加ToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            this.panel1.Visible = false;
            this.panel2.Visible = false;
            this.panel3.Visible = false;
        ...
        public void GetTeacherNo()
        {
            using (SqlConnection con = new SqlConnection())
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = con;
                cmd.CommandText = "select Teano from Teacher";
                SqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    this.cboBoxTeano.Items.Add(dr["Teano"].ToString());
                }
                this.cboBoxTeano.SelectedIndex = -1;
        ...
        {
            if (this.cboBoxTeano.SelectedIndex==-1||this.tBoxCno.Text==string.Empty||this.tBoxCname.Text==string.Empty||this.tBoxCredit.Text==string.Empty)
            {
                MessageBox.Show("请将课程信息填写完整");
                return;
            }
            using (SqlConnection con=new SqlConnection())
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand();
                    cmd.Connection = con;
                    cmd.CommandText = @" insert into Course values
                                          (
                                              @Cno,
                                              @Cname,
                                              @Teano,
                    ...
                    cmd.Parameters.AddWithValue("@Cno",this.tBoxCno.Text.Trim());
                    cmd.Parameters.AddWithValue("@Cname", this.tBoxCname.Text.Trim());
                    cmd.Parameters.AddWithValue("@Credit", this.tBoxCredit.Text.Trim());
                    cmd.Parameters.AddWithValue("@Teano", this.cboBoxTeano.SelectedItem.ToString().Trim());
                    cmd.ExecuteNonQuery();
        ...
        private void cboBoxTeano_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (this.cboBoxTeano.SelectedIndex!= -1)
            {
                using (SqlConnection con = new SqlConnection())
                {
                    con.ConnectionString = ConfigurationManager.AppSettings["SQL"].ToString();
                    ...
                    cmd.Parameters.AddWithValue("@Teano",this.cboBoxTeano.SelectedItem.ToString());
                    this.textBox6.Text = cmd.ExecuteScalar().ToString();
                    //SqlDataReader dr = cmd.ExecuteReader();
                    //while (dr.Read())
                    //{
                    //    this.cboBoxTeano.Items.Add(dr["Teano"].ToString());
        ...
        public void GetCourseNo()
        {
            using (SqlConnection con = new SqlConnection())
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = con;
                cmd.CommandText = "select Cno from Course";
                SqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    this.comboBox1.Items.Add(dr["Cno"].ToString());
                }
        ...
        {
             DialogResult result = MessageBox.Show("确定删除这门课吗？", "", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
             if (result == DialogResult.OK)
             {
                 using (SqlConnection con = new SqlConnection())
                 {
                     try
                     {
                         ...
                         cmd.Parameters.AddWithValue("@Cno",this.comboBox1.SelectedItem);
                         cmd.ExecuteNonQuery();
                         MessageBox.Show("删除成功");
                     }
                     catch (Exception)
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection())
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = con;
                cmd.CommandText = "select Cname,Credit,Teano from Course where Cno=@Cno";
                cmd.Parameters.AddWithValue("@Cno",this.comboBox1.SelectedItem.ToString());
                SqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    this.textBox7.Text=dr["Cname"].ToString();
                    this.textBox8.Text=dr["Credit"].ToString();
                    this.textBox9.Text=dr["Teano"].ToString();
                }
